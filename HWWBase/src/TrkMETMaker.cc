...
void TrkMETMaker::SetVars(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  *hyp_jets_p4 = HWWVal::hyp_jets_p4();
  ...
  for( unsigned int ihyp = 0 ; ihyp < nhyps ; ihyp++ ){
    ...
    HWWVal::trk_met().push_back    ( sqrt(metx*metx+mety*mety)  );
    HWWVal::trk_metPhi().push_back ( atan2(mety,metx)           );
